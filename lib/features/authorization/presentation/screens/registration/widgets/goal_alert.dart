...
class GoalAlert extends StatefulWidget {
  GoalAlert({Key? key}) : super(key: key);

  @override
  _GoalAlertState createState() => _GoalAlertState();
}

class _GoalAlertState extends State<GoalAlert> {
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('Выберите ваши цели'),
      content: SizedBox(
                ...
                builder: (context, formState) {
                  ...
                  if (formState is RegistrationFormInProgress ||
                    ...
                    return ListView.builder(
                      ...
                      itemCount: state.goals.isEmpty ? 0 : state.goals.length,
                              ...
                              onChanged: (bool? value) {
                                if (value == true) {
                                  context.read<RegistrationFormBloc>().add(
                                    GoalSelected(goal.Id),
                                  );
                                } else {
                                  context.read<RegistrationFormBloc>().add(
                                    GoalUnselected(goal.Id),
                                  );
                                }
                              },
                            ),
